...
def read_centroids(cnn, monitor_layer):
	...
	file = open(input_inference_HC_dir + "centroids_" + str(cnn) + "_" + str(monitor_layer) + ".txt", "r")
...
def read_threshold(cnn, monitor_layer):
	threshold = -1
	if ad_technique == "AE":
		file = open(input_inference_AE_dir + "mse_" + cnn + "_" + monitor_layer + ".txt", "r")
		threshold = float(file.readline())
		file.close()
	elif ad_technique == "HC":
		file = open(input_inference_HC_dir + "threshold_" + cnn + "_" + monitor_layer + ".txt", "r")
		threshold = float(file.readline())
		file.close()
	elif ad_technique == "SC":
		file = open(input_inference_SC_dir + "threshold_" + cnn + "_" + monitor_layer + ".txt", "r")
		threshold = float(file.readline())
		file.close()
	return threshold
...
def read_distribution(cnn, monitor_layer):
	...
	distribution = {}
	file = open(input_inference_SC_dir +  "distribution_" + str(cnn) + "_" + str(monitor_layer) + ".txt", "r")
	lines = file.readlines()
...
if mode == "training":
	...
	print(d_56_classifications)
	HOST = "127.0.0.1"  # The server's hostname or IP address
	PORT = 65483  # The port used by the server

	with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
    		s.connect((HOST, PORT))
    		if d_56_classifications.count("N")==1 :
    			s.sendall(b"N")
    		else:
    			s.sendall(b"A")
